!################################################################################
subroutine util_gexpihd(n, d, hmat, exph)
  ...
  exph(1:n, 1:n) = czero
  do k = 1, n
     expd = exp(iunit * d * htmp(k, k))
     !DEBUG
     !DEBUG write(6, "('util_gexpihd expd: ', i10, 2f20.10)") k, expd
     !DEBUG
     do i = 1, n
        do j = 1, n
